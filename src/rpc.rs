...
impl RPC {
    ...
    fn notify_subscriptions_for_tx(&self, txn: Transaction) -> Result<()> {
    ...
    pub fn notify_scripthash_subscriptions(
        ...
        for txid in txs_changed {
            ...
            notified.insert(txid);
            let tx = self.query.load_txn(&txid, None);
            if !tx.is_ok() {
                continue;
            }
            if let Err(e) = self.notify_subscriptions_for_tx(tx.unwrap()) {
        ...
        for header in headers_changed {
            ...
            let res = self.query.with_blocktxids(&blockhash, |txid| {
                ...
                notified.insert(*txid);
                let tx = self.query.load_txn(txid, Some(*blockhash));
                if let Err(e) = tx {
                    trace!("Failed to load tx {}: {}", txid, e);
                    return;
                }
                if let Err(e) = self.notify_subscriptions_for_tx(tx.unwrap()) {
                    trace!("Failed notifying subscriptions {}", e);
